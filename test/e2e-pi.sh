#!/usr/bin/env bash
# End-to-end checks for the native Pi tools (S8a), against a repo wired by the real `bin/avo
# install` and loaded by the real Pi resource loader — the same class `pi` itself uses. The unit
# tests prove the six tools behave; this proves Pi finds them, which is the half that breaks
# silently. Writes evidence/s8-e2e.txt.
set -uo pipefail
...
say "# avo S8a e2e — $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
[[ -L "$repo/.pi/extensions/avo" ]]
yes_no $? ".pi/extensions/avo is a symlink, not a copy" ".pi/extensions/avo is missing or is a copy"
[[ -f "$repo/.pi/extensions/avo/index.ts" ]]
yes_no $? "the link resolves to a real index.ts — pi loads <dir>/index.ts" "the link is dangling"
