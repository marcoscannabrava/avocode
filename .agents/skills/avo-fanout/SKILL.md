...
own agent process, and its own score. Nothing they do can reach your working tree until you promote
one, which is a separate command you run on purpose.

**This is a search, not a vote.** The probes explore; `f` measures; you choose; `avo commit` decides
whether the choice was progress. `avo fan` reports a `best` index — that is a hint drawn from the
scores, never a decision.

## When to fan out

Fan out when **you genuinely do not know which direction wins and the answer is cheap to measure**:
...
- **one direction is obviously right.** Just do it. N probes of the same idea cost N times as much
  and return the same answer.
- **`f` cannot tell the probes apart.** Without a scorer the probes come back unscored and you are
  reading N diffs by hand — usually slower than doing the work once.
- **the task needs the big model to even attempt.** Probes run on a small model on purpose (below).
  A probe that cannot make progress teaches you nothing about the direction.
...
big-model job.** N cheap probes tell you which direction is worth the expensive model; then the
main session does that direction properly.

```bash
export AVO_PROBE_MODEL=groq/llama-3.3-70b   # or pass --model
```

If you find yourself wanting the big model for every probe, you are not exploring — you are asking
for the work to be done four times. Do it once instead.
...
prompts — so write a prompt that names the *problem* and invites a direction, not one that dictates
the edit. A prompt that says "delete the bounds check" gets you four identical diffs.

Tell the probes what the loop needs back: what to change, that they must not touch `.avo/score`, and
that a one-line summary of *what they tried* is the thing you will read. Their final message becomes
`summary`.
...
other probes. If the patch needs a 3-way merge you are told so — check for conflict markers before
you score. The patch is kept at `.avo/worktrees/<run>/promote-<i>.patch` either way.

**Promote at most one probe per variation step.** Two probes' diffs stacked together is a candidate
neither of them measured, and `f` scored neither. If two directions both look good, promote one,
commit it, and fan out again from there — that is what the lineage is for.

**A losing probe is a measurement.** Before you clean up, record what the dead ends were:

```bash
avo mem add "unrolling the inner loop 4x: -12% on b1_s4096, probe 3 of run <id>"
```

That is the whole reason the probes were worth running. Without it you will re-derive the same three
dead ends next week — see [avo-lineage](../avo-lineage/SKILL.md).
...
- **Depth.** `AVO_FAN_DEPTH` (default 3) caps nesting. A probe at the limit is refused and must do
  the work itself. This is a **refusal (exit 1), not an error** — it is the harness telling you that
  you are the one who should be editing files now.
...
- **Never promote a probe you have not scored in the real tree.** A worktree score was measured
  against the baseline; your tree may have moved.
- **A probe must not edit `.avo/score`.** Say so in the prompt. A probe that changes `f` to pass is
  not a candidate, and every earlier version becomes incomparable.
