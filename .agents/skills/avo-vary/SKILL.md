...
function `f` (`.avo/score`) judges it. You do **not** decide what gets kept — `avo commit` does, and
it only keeps a candidate that beats the current best. Your job is to generate variation, not grade
it.

## The loop, in order

Do not skip steps 1–3. They stop you re-trying something that already failed.
...
| `no-op` | 0 | the tree is unchanged — you did not edit anything | make a change first |
...
A refusal is **not** a failure of the run — it is the commit rule working. The candidate stays in
your working tree, so you can iterate on it. Record what you learned first:

```bash
avo mem add "vectorizing the inner loop regressed b8_s1024 by 12% — the gather is the bottleneck, not the arithmetic"
```

`avo commit` already writes the refusal itself. Add an insight when you know *why* — the reason is
what stops the next session repeating it.

## Rules that matter

- **One idea per candidate.** Two changes in one commit and a refusal tells you nothing about which
  one cost you. Score them one at a time.
- **Never edit `.avo/score` to make a candidate pass.** That is gaming `f`, and it invalidates every
  earlier version in the lineage. If the scorer is genuinely wrong, fix it as its own committed change
  and say so in `--why`.
...
It reads the same lineage and attempt log you do, and fires on the two things you cannot see from
inside one turn:
...
- **thrash** — the last K attempts failed *the same way*. The same error three times means your
  diagnosis is wrong, not your edit.

The directive cites specific versions, the dead ends memory holds, and the docs in `K` no version has
ever mentioned. Treat that last list as your candidate directions — a doc is listed precisely because
nothing in the lineage talks about it.
...
the tree, asks `avo supervise` whether to steer, and spawns a *fresh* process for the next turn. Three
things follow:

- **You will not remember this turn.** The next iteration is a new process; its only context is the
  prompt it is given. Anything worth carrying forward must be written down — `avo commit --why`, or
  `avo mem add "<insight>"` for something not tied to a version.
...
- **`touch .avo/STOP` when the task is genuinely done.** It ends the loop before the next turn. Use it
  when there is nothing left to vary — not when *this* turn was hard.

Before calling `avo run` or `avo fan` from inside a turn, check the depth: `AVO_FAN_LEVEL` against
`AVO_FAN_DEPTH`. At the limit you must do the work yourself.
