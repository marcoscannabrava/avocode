 ...
 * the next prompt. That works for any agent (invariant 9) at a process per check. Pi does better:
 * `tool_result` fires the moment `avo_score` or `avo_commit` finishes — the only two events that can
 * change what `supervise()` reads — and `pi.sendMessage` injects without restarting anything.
 *
 * Three rules this file keeps:
 *
 *  1. **It counts the same attempts `avo supervise` counts**, from `.avo/attempts.jsonl` and the git
 *     lineage, never an in-session tally. Two drifting counters would steer one stall twice.
 *  2. **It re-implements nothing.** No commit rule, no threshold arithmetic, no directive
 *     rendering: `supervise()` owns all of it, as it does for the CLI (invariant 1).
 *  3. **It steers once per episode, not once per attempt.** See `episodeKeys`.
 *
 * Split from `pi/extensions/avo/` for the reason S7 split detector from driver: the tools test with
 * no session, this needs a scripted sequence of tool results. It also lets an operator load the
 * tools alone. Lives under `pi/` like `tools.ts`: `src/` must work in a Pi-less checkout.
 */
...
/**
 * The only tool results that can change what `supervise()` reads. `avo_score` appends to the attempt
 * log; `avo_commit` scores *and* may add a version. `avo_fan` is absent by design: it scores in
 * disposable worktrees with `record: false`, moving no counter here (invariant 7).
 */
...
/**
 * Records the injected directive, so a Pi-driven trajectory audits like an `avo run` one. Keyed by
 * episode rather than run-and-iteration: a session has no iteration number, and the episode key is
 * already unique per intervention, which makes re-recording idempotent (invariant 5).
 */
 ...
 *
 * Not every trigger: "5 attempts since the best" and "6 attempts since the best" say the same
 * thing, and re-injecting costs context and teaches the model to skim. Not once ever either: a
 * stall that ends and restarts is new, and so is a thrash appearing *during* a stall.
 *
 * The key is a pure function of the supervision state, so it needs no counter of its own and cannot
 * drift from what `avo supervise` would say (rule 1).
 ...
 *
 * `analyzed`, not `attempts`, is the anchor's base: past `ANALYSIS_WINDOW` the detector sees only a
 * window, so `attempts - since_best` would creep up once per attempt and re-steer forever.
 */
...
export function installSupervisor(pi: ExtensionAPI, deps: SupervisorDeps = defaultSupervisorDeps()): void {
  /**
   * Episodes already answered. A session-local *cache*, not state of record: the injected messages
   * hold everything needed to rebuild it, which is what makes branching work.
   */
  ...
  pi.on("session_start", (_event, ctx) => {
    closed = false;
    steered = new Set();
    // getBranch(), not getEntries(): off-branch entries are a path the model no longer remembers,
    // and counting them as answered would leave a real stall unsteered.
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    let s: Supervision;
    try {
      // Null thresholds so `.avo/config.json` decides, as `avo supervise` does with no flags.
      s = await deps.supervise({ json: true, cwd: repoOf(ctx), stall: null, thrash: null }, { runner: deps.runner });
    } catch (e) {
      // Invariant 4: an unreadable log must not take the session down — just the steering.
    ...
    try {
      ...
      // A bad memory write warns: the directive is already in the session file.
    ...
    // "steer" is right: the tool call that moved the score has just finished, so the directive
    // lands before the model's next decision. `triggerTurn` is deliberately unset — the supervisor
    // answers a turn the agent started, it does not start its own.
    pi.sendMessage({ customType: SUPERVISOR_MESSAGE_TYPE, content: s.directive, display: true, details }, { deliverAs: "steer" });
  });
}

/** `ctx.cwd` is the only source of the repo root — the tools' rule, for its reason. */
