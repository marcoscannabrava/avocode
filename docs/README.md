# avocode docs

One document per question. Start with whichever one you actually have.

| I want to… | Read |
| --- | --- |
| **install it** (and fix a broken PATH, or uninstall) | [install.md](install.md) |
| **understand what it does** and how the code is laid out | [architecture.md](architecture.md) |
| **look up a command** — every flag, every exit code | [commands.md](commands.md) |
| **wire my agent to it** — skills, `avo install`, the pi extensions | [agents.md](agents.md) |
| **run the tests** | [testing.md](testing.md) |
| **point the loop at a real target** and judge the curve | [bench.md](bench.md) |
| **write a scorer** — the `f` authoring guide | [../templates/score/README.md](../templates/score/README.md) |
| **see the plan** — slice order, invariants, open questions | [../PLAN.md](../PLAN.md) |
| know how this repo builds itself | [meta-loop.md](meta-loop.md) |

Background: [avo-paper.md](avo-paper.md) is the paper this harness is extracted from;
[avo-pi.md](avo-pi.md) is the original pi-flavored sketch.
