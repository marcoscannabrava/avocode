...
[`ralph`](https://github.com/marcoscannabrava/ralph) — it was `ralph.sh` here until it earned its own
repo, and it was never part of `avo` itself.
...
## Why its interrupt handling is load-bearing
...
([#33](https://github.com/marcoscannabrava/avocode/issues/33)). The fix, and the tests holding it in
place, live in the `ralph` repo now.

## One handoff trap, learned the hard way

**Never write `fixed`, `closes` or `resolves` next to an issue number in a commit message unless you
fixed it.** GitHub matches the keyword, not the sentence — one iteration's honest
